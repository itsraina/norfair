...
def draw_points(
    ...
    for d in detections:
        if color_by_label:
            color = Color.random(abs(hash(str(d.label))))
        points = d.points
...
def draw_tracked_objects(
    ...
    for obj in objects:
        if not obj.live_points.any():
            continue
        if color_by_label:
            point_color = Color.random(abs(hash(str(obj.label))))
    ...
    for obj in objects:
        ...
        if only_initializing_ids is not None:
            if obj.initializing_id not in only_initializing_ids:
                continue
        if color_by_label:
            text_color = Color.random(abs(hash(str(obj.label))))
        elif color is None:
...
def draw_boxes(
    ...
    for d in detections:
        if color_by_label:
            line_color = Color.random(abs(hash(str(d.label))))
        elif random_color:
...
def draw_tracked_boxes(
    ...
    for n, obj in enumerate(objects):
        if not obj.live_points.any():
            continue
        if color_by_label:
            color = Color.random(abs(hash(str(obj.label))))
